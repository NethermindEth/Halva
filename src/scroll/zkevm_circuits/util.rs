...
use halo2_proofs::{
    ...
    plonk::{Challenge, /* Circuit, */ ConstraintSystem, ErrorFront, Expression, FirstPhase, VirtualCells},
};
...
pub use crate::scroll::gadgets::util::Expr;
...
pub(crate) fn query_expression<T>(
    meta: &mut ConstraintSystem<TermField>,
    mut f: impl FnMut(&mut VirtualCells<TermField>) -> T,
) -> T {
    let mut expr = None;
...
//     }
    ...
    pub fn construct(meta: &mut ConstraintSystem<TermField>) -> Self {
        // #[cfg(any(not(feature = "onephase"), feature = "test", test))]
        let _dummy_cols = [
            meta.advice_column(),
            meta.advice_column_in(halo2_proofs::plonk::SecondPhase),
    ...
    pub fn exprs(&self, meta: &mut ConstraintSystem<TermField>) -> Challenges<Expression<TermField>> {
    ...
    pub fn values(&self, layouter: &impl Layouter<TermField>) -> Challenges<Value<TermField>> {
        Challenges {
...
//     fn new_from_block(block: &witness::Block) -> Self;
    ...
    fn instance(&self) -> Vec<Vec<TermField>> {
        vec![]
    }
...
pub trait SubCircuitConfig {
    ...
    /// Type constructor
    fn new(meta: &mut ConstraintSystem<TermField>, args: Self::ConfigArgs) -> Self;
}
