...
use super::keccak_packed_multi::keccak_unusable_rows;
use super::param::*;
...
pub(crate) fn pack(bits: &[u8]) -> TermField {
    pack_with_base(bits, BIT_SIZE)
}

/// Pack bits in the range [0,BIT_SIZE[ into a sparse keccak word with the
/// specified bit base
pub(crate) fn pack_with_base(bits: &[u8], base: usize) -> TermField {
    let base = TermField::from(base as u64);
    bits.iter()
        .rev()
        .fold(TermField::zero(), |acc, &bit| acc * base + TermField::from(bit as u64))
}
...
// TERMFIELD: cannot be done for symbolic values with non-symbolic return type
/// Unpack a sparse keccak word into bits in the range [0,BIT_SIZE[
pub(crate) fn unpack(_packed: TermField) -> [u8; NUM_BITS_PER_WORD] {
    [0; NUM_BITS_PER_WORD]
...
pub(crate) fn pack_u64(value: u64) -> TermField {
    pack(
        &((0..NUM_BITS_PER_WORD)
...
pub(crate) fn field_xor(a: TermField, b: TermField) -> TermField {
    // panic!("If field_xor is required it will require a custom implementation")
    TermField::from(format!("(field_xor ({a}) ({b}))"))
    ...
    //     bytes[idx] = *a ^ *b;
    // }
    // TermField::from_repr(bytes).unwrap()
}
...
pub(crate) mod compose_rlc {
    ...
    pub(crate) fn expr(expressions: &[Expression<TermField>], r: Expression<TermField>) -> Expression<TermField> {
...
pub(crate) mod scatter {
    ...
    pub(crate) fn expr(value: u8, count: usize) -> Expression<TermField> {
