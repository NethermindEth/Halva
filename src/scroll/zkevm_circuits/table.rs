...
//     },
    plonk::{Advice, Any, Column, ConstraintSystem, /*Error,*/ ErrorFront, /*Expression,*/ Fixed, /*VirtualCells*/},
//     poly::Rotation,
};
...
//     }
    ...
    /// columns.
    fn annotate_columns(&self, cs: &mut ConstraintSystem<TermField>) {
        self.columns()
            .iter()
    ...
    fn annotate_columns_in_region(&self, region: &mut Region<TermField>) {
        self.columns()
            .iter()
            .zip(self.annotations().iter())
...
impl LookupTable for KeccakTable {
    fn columns(&self) -> Vec<Column<Any>> {
        vec![
            self.q_enable.into(),
            self.is_final.into(),
...
//     }
    ...
    /// NOTE: values are assigned to advice columns
    pub fn assign_row(
        &self,
        region: &mut Region<TermField>,
        offset: usize,
        values: [Value<TermField>; 4],
    ) -> Result<Vec<AssignedCell<TermField, TermField>>, ErrorFront> {
        let mut res = vec![];
        for (&column, value) in <KeccakTable as LookupTable>::advice_columns(self)
            .iter()
            .zip(values.iter())
        {
